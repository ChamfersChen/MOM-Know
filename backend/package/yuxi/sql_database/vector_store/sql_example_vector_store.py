...
SQL_EXAMPLE_COLLECTION = "sql_examples"
VECTOR_METRIC_TYPE = "COSINE"
CONTENT_ANALYZER_PARAMS = {"type": "chinese"}


class SqlExampleVectorStore:
    """SQL 示例的向量存储管理。

    在 Milvus 中创建 `sql_examples` 集合，支持语义/关键词/混合检索。
    """

    def __init__(self):
        self.connection_alias = f"sql_example_milvus_{hashstr(SQL_EXAMPLE_COLLECTION, 6)}"
    ...
    def _connect(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"SqlExampleVectorStore: database operation failed, using default: {exc}")
            self._connected = True
            logger.info(f"SqlExampleVectorStore connected to Milvus at {uri}")
        except Exception as exc:
            logger.error(f"SqlExampleVectorStore failed to connect to Milvus: {exc}")
            raise
    ...
    def _create_collection(self, embedding_dim: int):
        ...
        schema = CollectionSchema(
            fields=fields,
            description="SQL examples for semantic search",
            functions=[bm25_function],
        )
        collection = Collection(name=SQL_EXAMPLE_COLLECTION, schema=schema, using=self.connection_alias)
        ...
        logger.info(f"Created Milvus collection {SQL_EXAMPLE_COLLECTION} with dim={embedding_dim}")
        return collection

    def _get_or_create_collection(self, embedding_dim: int) -> Collection:
        self._connect()
        if utility.has_collection(SQL_EXAMPLE_COLLECTION, using=self.connection_alias):
            collection = Collection(name=SQL_EXAMPLE_COLLECTION, using=self.connection_alias)
            ...
            if dim_field is not None and dim_field.params.get("dim") == embedding_dim and has_enabled:
                logger.info(f"Reusing existing Milvus collection {SQL_EXAMPLE_COLLECTION}")
                return collection
            logger.warning(
                f"Schema changed (dim_match={dim_field is not None and dim_field.params.get('dim') == embedding_dim}, "
                f"has_enabled={has_enabled}), dropping and recreating {SQL_EXAMPLE_COLLECTION}"
            )
            utility.drop_collection(SQL_EXAMPLE_COLLECTION, using=self.connection_alias)
        return self._create_collection(embedding_dim)

    async def clear_all(self):
        """删除并重建 sql_examples 集合，清空所有数据。"""
        try:
            self._connect()
            if utility.has_collection(SQL_EXAMPLE_COLLECTION, using=self.connection_alias):
                self.collection = None
                await asyncio.to_thread(utility.drop_collection, SQL_EXAMPLE_COLLECTION, using=self.connection_alias)
            await self.initialize_collection()
            logger.info("Cleared and recreated sql_examples collection")
        except Exception as exc:
            logger.warning(f"Failed to clear sql_examples collection: {exc}")
    ...
    async def index_sql(
        ...
        content = f"{sql} {description}" if description else sql
        ...
        logger.info(f"Indexed SQL example {row_id} into Milvus")
    ...
    async def batch_index(
        ...
        sqls = [e.get("sql", "") for e in entries]
        ...
        logger.info(f"Batch indexed {res.insert_count}/{len(entries)} SQL examples into Milvus")

    async def remove_sql(self, row_id: int):
        if self.collection is None:
            return
        self.collection.delete(f'id == "{row_id}"')
    ...
    async def search(
        ...
        output_fields = ["id", "sql", "description", "datasource_host", "datasource_port", "enabled"]
        results: list[dict] = []
