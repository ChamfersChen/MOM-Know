...
sql_kb = APIRouter(prefix="/sql_knowledge", tags=["knowledge"])
...
@sql_kb.get("/databases")
async def get_databases(
    # current_user: User = Depends(get_admin_user)
    ):
    """获取所有知识库"""
    try:
        database = sql_knowledge_base.get_databases()
        return database
...
async def create_database(
    ...
    try:
        ...
        database_info = await sql_knowledge_base.create_database(
...
@sql_kb.get("/databases/{db_id}")
async def get_database_info(
    db_id: str, 
    # current_user: User = Depends(get_admin_user)
    ):
    """获取知识库详细信息"""
    database = sql_knowledge_base.get_database_info(db_id)
    if database is None:
        raise HTTPException(status_code=404, detail="Database not found")
    return database


@sql_kb.put("/databases/{db_id}")
async def update_database_info(
    ...
    try:
        database = await sql_knowledge_base.update_database(
            db_id,
            name,
            description,
            llm_info,
...
async def delete_database(
    ...
    try:
        await sql_knowledge_base.delete_database(db_id)
...
@sql_kb.get("/databases/{db_id}/export")
async def export_database(
    ...
    try:
        file_path = await sql_knowledge_base.export_data(db_id, format=format, include_vectors=include_vectors)
        ...
        try:
            ...
            for idx, item in enumerate(items, 1):
                ...
                try:
                    ...
                    file_meta = await sql_knowledge_base.add_file_record(
            ...
            for idx, (item, (file_id, add_file_meta)) in enumerate(added_files.items(), 1):
                ...
                try:
                    ...
                    file_meta = await sql_knowledge_base.parse_file(db_id, file_id, operator_id=current_user.id)
...
async def check_db_and_tables(
    ...
    """检查数据库是否存在并返回表名"""
    try:
        res, field_res = sql_knowledge_base.check_mysql_connection(host, user, password, database, port)
        return {'status': 'success', 'result': res, 'field_result': field_res}
    except Exception as e:
        return {"status": "failed", "message": str(e)}

@sql_kb.post("/databases/{db_id}/upload/database")
async def process_db_and_upload(
    ...
    # 1. 从数据库获取文本数据
    text_data = sql_knowledge_base.get_data_from_db(host, user, password, database, port, table, fields)  # 你的数据库查询函数
    # 2. 创建临时文件
    ...
    try: 
        ...
        if db_id:
            upload_dir = sql_knowledge_base.get_db_upload_path(db_id)
        else:
        ...
        file_exists = await sql_knowledge_base.file_existed_in_db(db_id, content_hash)
        if file_exists:
            raise HTTPException(
                status_code=409,
...
@sql_kb.post("/databases/{db_id}/documents")
...
@sql_kb.post("/databases/{db_id}/documents/parse")
    ...
    async def run_parse(context: TaskContext):
        ...
        try:
            for idx, file_id in enumerate(file_ids, 1):
                ...
                try:
                    result = await sql_knowledge_base.parse_file(db_id, file_id, operator_id=current_user.id)
...
@sql_kb.post("/databases/{db_id}/documents/index")
        ...
        try:
            ...
            if params:
                for file_id in file_ids:
                    try:
                        await sql_knowledge_base.update_file_params(db_id, file_id, params, operator_id=operator_id)
                    except Exception as e:
            ...
            for idx, file_id in enumerate(file_ids, 1):
                ...
                try:
                    result = await sql_knowledge_base.index_file(db_id, file_id, operator_id=operator_id)
                    processed_items.append(result)
                except Exception as e:
...
@sql_kb.get("/databases/{db_id}/documents/{doc_id}")
async def get_document_info(db_id: str, doc_id: str, current_user: User = Depends(get_admin_user)):
    ...
    try:
        info = await sql_knowledge_base.get_file_info(db_id, doc_id)
        return info
    except Exception as e:
...
@sql_kb.get("/databases/{db_id}/documents/{doc_id}/basic")
async def get_document_basic_info(db_id: str, doc_id: str, current_user: User = Depends(get_admin_user)):
    ...
    try:
        info = await sql_knowledge_base.get_file_basic_info(db_id, doc_id)
        return info
    except Exception as e:
...
@sql_kb.get("/databases/{db_id}/documents/{doc_id}/content")
async def get_document_content(db_id: str, doc_id: str, current_user: User = Depends(get_admin_user)):
    ...
    try:
        info = await sql_knowledge_base.get_file_content(db_id, doc_id)
        return info
    except Exception as e:
...
@sql_kb.delete("/databases/{db_id}/documents/{doc_id}")
async def delete_document(db_id: str, doc_id: str, current_user: User = Depends(get_admin_user)):
    ...
    try:
        file_meta_info = await sql_knowledge_base.get_file_basic_info(db_id, doc_id)

        # Check if it is a folder
        is_folder = file_meta_info.get("meta", {}).get("is_folder", False)
        if is_folder:
            await sql_knowledge_base.delete_folder(db_id, doc_id)
        ...
        await sql_knowledge_base.delete_file(db_id, doc_id)
        return {"message": "删除成功"}
    except Exception as e:
...
@sql_kb.get("/databases/{db_id}/documents/{doc_id}/download")
async def download_document(db_id: str, doc_id: str, request: Request, current_user: User = Depends(get_admin_user)):
    ...
    try:
        file_info = await sql_knowledge_base.get_file_basic_info(db_id, doc_id)
...
@sql_kb.post("/databases/{db_id}/query")
async def query_knowledge_base(
    ...
    try:
        result = await sql_knowledge_base.aquery(query, db_id=db_id, **meta)
...
@sql_kb.post("/databases/{db_id}/query-test")
...
@sql_kb.post("/databases/{db_id}/query-for-bot")
...
@sql_kb.put("/databases/{db_id}/query-params")
async def update_knowledge_base_query_params(
    ...
    try:
        # 获取知识库实例
        kb_instance = sql_knowledge_base.get_kb(db_id)
        if not kb_instance:
            raise HTTPException(status_code=404, detail="Knowledge base not found")

        # 更新实例元数据中的查询参数
        async with sql_knowledge_base._metadata_lock:
...
@sql_kb.get("/databases/{db_id}/query-params")
async def get_knowledge_base_query_params(db_id: str, current_user: User = Depends(get_admin_user)):
    """获取知识库类型特定的查询参数"""
    try:
        # 获取知识库实例
        kb_instance = sql_knowledge_base._get_kb_for_database(db_id)
...
@sql_kb.post("/databases/{db_id}/sample-questions")
async def generate_sample_questions(
    ...
    try:
        ...
        # 获取知识库信息
        db_info = sql_knowledge_base.get_database_info(db_id)
        if not db_info:
        ...
        try:
            ...
            try:
                async with sql_knowledge_base._metadata_lock:
                    # 确保知识库元数据存在
                    if db_id not in sql_knowledge_base.global_databases_meta:
                        sql_knowledge_base.global_databases_meta[db_id] = {}
                    # 保存问题到对应知识库
                    sql_knowledge_base.global_databases_meta[db_id]["sample_questions"] = questions
                    sql_knowledge_base._save_global_metadata()
...
@sql_kb.get("/databases/{db_id}/sample-questions")
async def get_sample_questions(db_id: str, current_user: User = Depends(get_admin_user)):
    ...
    try:
        ...
        db_meta = sql_knowledge_base.global_databases_meta[db_id]
...
@sql_kb.post("/databases/{db_id}/folders")
async def create_folder(
    ...
    """创建文件夹"""
    try:
        return await sql_knowledge_base.create_folder(db_id, folder_name, parent_id)
    except Exception as e:
        logger.error(f"创建文件夹失败 {e}, {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=str(e))


@sql_kb.put("/databases/{db_id}/documents/{doc_id}/move")
async def move_document(
    ...
    try:
        return await sql_knowledge_base.move_file(db_id, doc_id, new_parent_id)
    except ValueError as e:
...
async def upload_file(
    ...
    # 检测同名文件（基于原始文件名）
    same_name_files = await sql_knowledge_base.get_same_name_files(db_id, filename)
...
@sql_kb.get("/files/supported-types")
...
async def get_knowledge_base_types(current_user: User = Depends(get_admin_user)):
    """获取支持的知识库类型"""
    try:
        kb_types = sql_knowledge_base.get_supported_kb_types()
...
async def get_knowledge_base_statistics(current_user: User = Depends(get_admin_user)):
    """获取知识库统计信息"""
    try:
        stats = sql_knowledge_base.get_statistics()
...
@sql_kb.get("/embedding-models/{model_id}/status")
...
@sql_kb.get("/embedding-models/status")
...
@sql_kb.post("/generate-description")
