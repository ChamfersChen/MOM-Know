...
import { knowledgeApi, documentApi, queryApi } from '@/apis/sql_knowledge_api';
import { useTaskerStore } from '@/stores/tasker';
import { useRouter } from 'vue-router';
import { parseToShanghai } from '@/utils/time';

export const useSqlKnowledgeStore = defineStore('sql_knowledge', () => {
  ...
  async function createDatabase(formData) {
    ...
    state.creating = true;
    try {
      const data = await knowledgeApi.createDatabase(formData);
  ...
  async function getDatabaseInfo(id, skipQueryParams = false, isBackground = false) {
    ...
    try {
      const data = await knowledgeApi.getDatabaseInfo(db_id);
      database.value = data;
  ...
  async function updateDatabaseInfo(formData) {
    try {
      state.lock = true;
      await knowledgeApi.updateDatabase(databaseId.value, formData);
  ...
  function deleteDatabase() {
    Modal.confirm({
      ...
      onOk: async () => {
        state.lock = true;
        try {
          const data = await knowledgeApi.deleteDatabase(databaseId.value);
  ...
  async function loadQueryParams(id) {
    const db_id = id || databaseId.value;
    console.log('>>> loadQueryParams: ', db_id)
    if (!db_id) return;
